...
angular.module("m1cp").controller('headerController', function ($scope, $timeout, $window) {
...
angular.module("m1cp").controller('footerController', function ($scope, $timeout, $window) {
...
angular.module("m1cp").controller('arrowClickController', function ($scope, $timeout, $window) {
...
// slide to section-animation for moving to next section
angular.module("m1cp").controller('commonController', function ($scope, $timeout, $window) {
});

angular.module("m1cp").directive("backgroundImg", function ($timeout) {
    return {
        restrict: 'A',
...
angular.module("m1cp").directive("carouselComponent", function ($timeout) {
    return {
        restrict: "A",
        ...
        link: function (scope, element, attrs) {
            ...
            function iconpossition() {
                ...
                var dottedImage = element[0].querySelector('.border-dottedimage,.Carousel-deviceDetailsImage');
...
angular.module("m1cp").directive("viewAll", function ($timeout) {
    return {
        restrict: 'A',
...
angular.module("m1cp").directive("megamenuArrow", function ($timeout) {
     return {
        restrict: 'A',
        link: function (scope, elem, attrs) {
...
angular.module("m1cp").directive("deviceviewAll", function ($timeout) {
    return {
        restrict: 'A',
        link: function (scope, element, attrs, $window) {
            element.bind("click", function () {
...
angular.module("m1cp").directive("equalHeight", function ($timeout) {
    return {
        restrict: 'A',
        link: function (scope, element, attrs, $window) {

            angular.element(document).ready(function () {
                length = element.find(".description p").length;
                minheight = 0;
                maxheight = 1;
                for (var i = 0; i < length; i++) {
                    var dom = element.find(".description p")[i];
                    var height = angular.element(dom).height();
                    if (minheight <= maxheight) {
                        minheight = maxheight;
                        maxheight = height;
                    }
                }
                element.find(".description p").css("height", minheight + "px");
            });
        }
    }
    });
